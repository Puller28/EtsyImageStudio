...
## Recent Changes
- **Performance Optimization (August 20, 2025)**: Fixed critical performance issue where recent projects were taking 2+ minutes to load. Added timeout handling (30 seconds) to database operations and improved logging in RobustStorage fallback mechanism. Database index created on projects table for user_id + created_at to improve query performance.
